 ...
 * URL : https://dashboard.test/widgets/gmail/callback.php
 */
...
// Stockage des tokens
$db->setSetting('gmail', 'refresh_token',           $response['refresh_token']);
$db->setSetting('gmail', 'access_token',            $response['access_token'] ?? '');
$db->setSetting('gmail', 'access_token_expires_at', (string) (time() + ($response['expires_in'] ?? 3600)));

// Vider le cache du widget
$cache = new Cache();
$cache->deleteByPrefix('widget_gmail');
